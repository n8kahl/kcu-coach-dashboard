...
/**
 * Achievement Card Modal - Full screen modal for celebrating achievements
 */
export function AchievementCardModal({
  isOpen,
  onClose,
  ...props
}: AchievementCardProps & { isOpen: boolean }) {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="relative w-full max-w-md mx-4 animate-in fade-in zoom-in duration-300">
        <AchievementCard {...props} onClose={onClose} className="shadow-2xl" />
      </div>
    </div>
  );
}
...
export function useWinCardTrigger() {
  const [winCard, setWinCard] = useState<{
    type: AchievementCardProps['type'];
    title: string;
    description: string;
    stats: AchievementCardProps['stats'];
  } | null>(null);
