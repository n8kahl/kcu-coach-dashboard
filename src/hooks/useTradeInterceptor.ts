...
export function useTradeInterceptor(
    ...
    try {
      ...
      if (!result.approved) {
        ...
        showToast({
          type: 'error',
          title: result.title,
          message: result.blockedReason || 'Trade blocked by coach',
        });
        ...
        showToast({
          ...
          message: result.warnings[0] || result.message.slice(0, 100),
  ...
  }, [buildContext, showToast, onApproved, onRejected, cautionAutoDismissMs]);
  ...
  const forceApprove = useCallback(() => {
    if (pendingIntent && state.status === 'caution') {
      showToast({
        ...
        message: 'Trade approved with warning override. Be careful.',
        duration: 3000,
      });
  ...
  }, [pendingIntent, state, showToast, onApproved]);
